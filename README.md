...
| Zero-terminal switch-on: migrations self-apply at boot (schema_migrations ledger), ADMIN_KEY + VAPID self-provision and persist, owner reads the master key in Ops, reminders + digest run on an in-app clock with atomic claims | ✅ tested + dress-rehearsed against a real PostgreSQL 16 (fresh boot, restart, key persistence) |
...
| iPhone sound: the tutor's voice is only routed through the audio engine when that engine is genuinely awake (Safari starts it asleep, and routing into a sleeping engine silences the voice entirely), the first touch wakes it, both spellings of the engine are accepted, and the voice element plays inline | ✅ pure rules unit-tested + proven in a real production build driven at phone size against a Safari-like engine: the pre-fix build fails the same probe (`tools/device/audio-probe.mjs`) |
